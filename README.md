...
为了复现 Spark UI 的 task time 分解，Driver 按 `(stageId, stageAttemptId)` 生成 Scheduler Delay、Task Deserialization、Shuffle Read、Executor Computing、Shuffle Write、Result Serialization 和 Getting Result，分别写入 `spark.stage.task_metrics.timeline.scheduler_delay_ms`、`executor_deserialize_time_ms`、`shuffle_read_time_ms`、`executor_computing_time_ms`、`shuffle_write_time_ms`、`result_serialization_time_ms` 和 `getting_result_time_ms`。Task Deserialization、Shuffle Read fetch wait 和 Result Serialization 在 stage 完成时直接取 `StageInfo.taskMetrics` 的累计值；其余四段仍在 `SparkListenerTaskEnd` 上逐 task 计算后累加，以保留 Scheduler/Getting Result 所需时间戳、Executor Computing 的逐 task clamp 和 Shuffle Write 的逐 task 毫秒截断。`observed_task_attempts` 与 `included_task_attempts` 只描述这四个逐 task 分段的覆盖度；`StageInfo.taskMetrics` 不可用或完全没有带 `TaskMetrics` 的已观察 task attempt 时，整组 timeline 属性省略。Application Detail 的 `Stage task time distribution` 以普通堆叠条形图显示这七段累计工作时间，并显式区分 stage attempt；Stage 标签以 `TaskEnd included/observed` 展示四个逐 task 分段的覆盖度。由于三段来自 Spark stage accumulator、四段来自 TaskEnd attempt，失败、重试或推测执行时各段的贡献 task 集合可能不同。它不是实际时序或 stage 关键路径，Shuffle Read 具体表示 fetch wait，Executor Computing 仍包含 GC 等 executor 工作。面板通过 Tempo span-table 查询，`spss=100` 将每个返回 span set 中匹配的 stage span 限制为最多 100 个。

Application Detail 的 `Stage metrics` 表按 stage attempt 展示上述 `StageInfo.taskMetrics` 快照中的累计 executor runtime、input/output、shuffle read/write 和 spill size，并在 Grafana 中派生三个 bytes/s 比率：`(input bytes + shuffle read bytes) / executor runtime`、shuffle read bytes / fetch wait、shuffle write bytes / write time。它们是基于累计 task 时间的诊断比率，不是 stage 墙钟吞吐；尤其 shuffle read bytes 同时包含 local/remote 数据，而 fetch wait 只表示 remote fetch 的阻塞等待。分母为零时相应比率没有定义，表中显示为 `N/A`。该表只列出带 `StageTaskMetrics` 的已完成 stage attempt，同样受 `spss=100` 的最多 100 个匹配 stage span 限制。
